...
高さ：`45dp`
<br>
実際に使用する場合は、数値をハードコーディングするのではなく、下記のdimen.xmlのように変数名を使用すること。
...
色：用途に応じて変更(ex.`#16A6B6`)
<br>
色はそれぞれのアクションや画面ごとに沿ったものを使用します。
<br>
実際に使用する場合は、カラーコードをハードコーディングするのではなく、変数名を使用すること。
<br>
※ 詳細はColorの項目
...
ポジション：`left`
<br>
アイコン：ボタンの種類によって適したアイコンを使用すること。

③ text

テキスト：ボタンのアクションに沿って適した内容を使用すること。
<br>
実際に使用する場合は、ハードコーディングするのではなく、変数名を使用すること。

ポジション：`center`

テキストカラー：白。
<br>
ただし、下地が白の場合は、グレーを使用すること。
...
また画面の幅や高さに適さないサイズ、テキストサイズ、色の変更も禁止します。
...
* 基本的にStationアプリ内では、色によってボタンやヘッダーの種類や役割を区別します。
...
基本的に背景にアクセントカラーの設定はハーレーションを起こすなど、ユーザーに不快感を与える可能性があるため禁止します。
<br>
カラーコードのハードコーディングは禁止します。
